...
<div class="search-area">
	<button type="button" class="search-btn btn btn-warning" id="restore-btn">복원</button>
	<button type="button" class="search-btn btn btn-danger" id="delete-btn">영구삭제</button>
</div>
		...
		<c:if test="${not empty list }">
			<c:forEach items="${list}" var="mail">
				<tr id="listtr" data-messageid="${mail.mailMessageId}">
					...
					<td class="to">
						<c:forEach items="${mail.delmailToList}" var="ToOne">
							<span class="mail-span badge bg-label-info">${ToOne}</span>
						</c:forEach>
					</td>	
							
					<td class="from">
						<c:forEach items="${mail.delmailFromList}" var="fromOne">
							<span class="mail-span badge bg-label-info">${fromOne}</span>
						</c:forEach>
					</td>
					
							
					<td class="clickTitle">${mail.delmailSubject}</td>			
					<td>${mail.delmailDate}</td>			
				</tr>
			</c:forEach>
		</c:if>
		<c:if test="${empty list}">
  ...
  <div class="modal-dialog modal-xl">
    <div class="modal-content">
      ...
      <div class="modal-body">
        <table>
        	<tr style="display:none"  id="this-message-id">
        		...
        		<td class="modal-body-main" id="mail-id"></td>
        	</tr>
        	<tr>
      ...
      <div class="modal-footer">
        <button type="button" id="modal-restore-btn" class="btn btn-warning btn-sm">복원</button>
        <button type="button" id="modal-del-btn" class="btn btn-danger btn-sm">영구삭제</button>
...
<script type="text/javascript" src="${pageContext.request.contextPath}/resources/js/app/mail/mailDeleteList.js"></script>