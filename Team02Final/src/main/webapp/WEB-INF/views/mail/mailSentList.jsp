		...
		<c:if test="${not empty list }">
			<c:forEach items="${list}" var="mail">
				<tr id="listtr" data-messageid="${mail.mailMessageId}">
					...
					<td class="to">
						<c:forEach items="${mail.smailToList}" var="toOne">
							<span class="mail-span badge bg-label-info">${toOne}</span>
						</c:forEach>
					</td>			
					<td class="clickTitle">${mail.smailSubject}</td>			
					<td>${mail.smailDate}</td>			
				</tr>
			</c:forEach>
		</c:if>
		<c:if test="${empty list}">
...
<div class="modal fade" tabindex="-1" id="mail-detail"
	aria-hidden="true">
...
<script type="text/javascript" src="${pageContext.request.contextPath}/resources/js/app/mail/mailSentList.js"></script>