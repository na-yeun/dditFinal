...
<div class="search-area">
	<button type="button" class="search-btn btn btn-warning" id="important-btn">중요</button>
	...
	<thead>
		<tr>
			...
			<c:if test="${not empty list }">
				<th id="my-mail-area" data-mymail="${list[0].empMail}" class="from th-tofrom">발신자</th>
			</c:if>
			<c:if test="${empty list }">
				<th id="my-mail-area" data-mymail="" class="from th-tofrom">발신자</th>
			</c:if>
		...
		<c:if test="${not empty list}">
			<c:forEach items="${list}" var="mail">
				<tr id="listtr" data-messageid="${mail.mailMessageId}">
					...
					<td class="from">
						<c:forEach items="${mail.rmailFromList}" var="fromOne">
							<span class="mail-span badge bg-label-info">${fromOne}</span>
						</c:forEach>
					</td>			
					<td class="clickTitle">${mail.rmailSubject}</td>			
					<td>${mail.rmailDate}</td>			
				</tr>
			</c:forEach>
		</c:if>
		<c:if test="${empty list}">
  ...
  <div class="modal-dialog modal-xl">
    <div class="modal-content">
      ...
      <div class="modal-footer">
        ...
        <button type="button" id="modal-important-btn" class="btn btn-warning btn-sm">중요</button>
...
<script type="text/javascript" src="${pageContext.request.contextPath}/resources/js/app/mail/mailReceivedList.js"></script>