...
document.addEventListener('DOMContentLoaded', () => {
	...
	$(document).ready(function() {
		const url = `${contextPathMessage}/${companyId}/directory/folder`; // url 변수 정의
		...
		function getJson(searchKeyword = '') {
			$.ajax({
				...
				success: function(data) {
					...
					if (tree) {
						tree.settings.core.data = directory.map(node => ({
							id: node.id,
							parent: node.parent,
							text: node.text,
							icon: node.icon,
		...
		Swal.fire({
			...
			if (result.isConfirmed) {
				  ...
				  const payload = {
					...
					receivers: JSON.stringify(receivers), // 수신자 리스트를 JSON 문자열로 변환
				};
			
				$.ajax({
					url: `${contextPathMessage}/${companyId}/message/send`,
					method: 'POST',
					data: payload,
