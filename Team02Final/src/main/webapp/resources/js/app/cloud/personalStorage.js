...
document.addEventListener("DOMContentLoaded", ()=>{
    ...
    pasteBtn.addEventListener("click", async () => {
        ...
        // 서버로 이동 요청
        fetch(`${contextPath}/${companyId}/perCloud/moveObjects`, {
            method: "POST",
            headers: {
    ...
    searchBtn.addEventListener("click", (e) => {
        ...
        if (searchKeyWord.length === 0) {
            ...
            // 서버에 검색 요청
            fetch(`${contextPath}/${companyId}/perCloud/searchObjects`, {
                method: "POST",
                headers: {
    ...
    deleteBtn.addEventListener("click", async (e)=>{
        ...
        if(filterAns == true){
           ...
           fetch(`${contextPath}/${companyId}/perCloud/deleteObjects`, {
                method : "POST",
                headers: {
                    'Content-Type': 'application/json'
                },
    ...
    if (selected.length > 1) {
        ...
        .then(blob => {
            ...
            a.href = url;
            a.download = "개인클라우드.zip"; // 다운로드 파일 이름
    ...
    fileInput.addEventListener("change", async () => {
        ...
        fetch(`${contextPath}/${companyId}/perCloud/upload`, {
            method: "POST",
            body: formData,
        })
        .then(resp => {
...
function navigateToFolder(folderName){
    ...
    currentPathForm.value = "MyCloud:/"+path;
...
function updateTable(folders, files) {
    ...
    sameFilesCheck = files;
    sameFolderCheck = folders;
...
function thumbnail(object){
    ...
    fetch(`${contextPath}/${companyId}/perCloud/imageThumbnail`, {
        method: "POST",
        headers: {
            'Content-Type': 'application/x-www-form-urlencoded'
        },
...
function navigateToParentFolder() {
    ...
    currentPathForm.value = "MyCloud:/"+path.slice(0, -1);
