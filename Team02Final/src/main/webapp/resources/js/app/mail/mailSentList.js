...
document.addEventListener("DOMContentLoaded", function() {
	...
	// 한 명 이상일 경우 
	// 모든 class="to" 요소 선택
	const cells = document.querySelectorAll(".to"); 
	...
	// 메일 클릭시 데이터 가지고 와서 모달 열리는 이벤트(상세정보)
	mailSelector.forEach(function(myMail) {
		myMail.addEventListener("click", async function(e) {
			...
			let resp = await fetch(url);
			if (resp.ok) {
				let { mailMessageId, empMail, smailToList, smailCcList, smailBccList, smailDate, smailSubject, mailAttachmentList, smailContent } = await resp.json();
				document.querySelector('#mail-id').value = mailMessageId;
				document.querySelector('#mail-from').innerHTML = `나 &lt;${empMail}&gt;`;
				
				let toCode = ``;
				smailToList.forEach(toOne => {
				...
				document.querySelector('#mail-date').innerHTML = smailDate;
				document.querySelector('#mail-title').innerHTML = smailSubject;
				document.querySelector('#mail-content').innerHTML = smailContent;

				let attachCode = '';

				mailAttachmentList.forEach((mailAttachment, i) => {
					if (mailAttachment.mailattachmentName) {
						attachCode += `<a href='javascript:void(0)' onclick="getAttach('${mailAttachment.mailmessageId}','${mailAttachment.mailattachmentId}')">${mailAttachment.mailattachmentName}</a><br/>`;
					}
				})

				document.querySelector('#attach-area').innerHTML = attachCode;
