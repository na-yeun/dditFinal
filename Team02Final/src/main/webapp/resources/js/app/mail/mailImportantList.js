...
// 메일 중요 설정 해제 함수
async function disimportantMail(checkedIds){
	// 아이디들이 담긴 checkedIds를 controller에 post 방식으로 보내기.. 비동기로
	// 배열에 담긴 message id를 통해 삭제메일함 insert, 해당 메일함 delete처리 + api trash
	let url = window.location.pathname + '/disimportant';
	let resp = await fetch(url, {
		method: 'POST',
		headers: {
			'Content-Type': 'application/json'  // JSON 형식으로 보내기
...
document.addEventListener("DOMContentLoaded", function () {
	...
	mailSelector.forEach(function(myMail) {
		myMail.addEventListener("click", async function(e) {
			...
			let resp = await fetch(url);
			if (resp.ok) {
				let { mailMessageId, imailFrom, empMail, imailToList, imailCcList, imailBccList, imailFromList, imailDate, imailSubject, mailAttachmentList, imailContent } = await resp.json();
				
				if(imailFrom==empMail){
					// 보낸 사람과 나의 메일이 같으면 답장기능 없어도 됨
					document.querySelector('#modal-re-btn').style.display = 'none';
				} else {
					document.querySelector('#modal-re-btn').style.display = 'block';
				}
				
				document.querySelector('#mail-id').value = mailMessageId;
				// from
				let fromCode = ``;
				imailFromList.forEach(fromOne => {
				...
				document.querySelector('#mail-date').innerHTML = imailDate;
				document.querySelector('#mail-title').innerHTML = imailSubject;
				document.querySelector('#mail-content').innerHTML = imailContent;
	...
	mailImportantBtn.addEventListener("click", () => {
		...
		if (checkedCheckBoxes.length != 0) {
			Swal.fire({
				title: "일반메일로 전환하시겠습니까?",
				html: "일반메일로 전환시 <br><b>받은 메일함 혹은 보낸메일함</b>에서 확인할 수 있습니다.",
				icon: "warning",
